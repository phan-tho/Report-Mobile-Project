...
\begin{figure}[H]
    ...
    \includegraphics[width=0.8\textwidth]{usecase_tong.png}
...
\textbf{Giải thích chi tiết:}
Từ màn hình chi tiết, khi người dùng nhấn nút "Thêm vào Thư viện", \texttt{MangaDetailScreen} gửi một sự kiện \texttt{toggleLibraryStatus()} tới \texttt{MangaDetailViewModel}. Nhận được tín hiệu, \texttt{ViewModel} gọi phương thức \texttt{insertMangaToLibrary()} của \texttt{LibraryRepository}. \texttt{Repository} sẽ tạo một \texttt{LibraryItemEntity} và dùng \texttt{LibraryDao} để thực thi truy vấn \texttt{@Upsert} vào Room Database. 
